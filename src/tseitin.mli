...
(** Conversion functions from boolean gates to Tseitin form.  
    The first argument to each function is the newly introduced
    sat literal for this gate, which should be referenced by its
    fanouts. *)
module Make(B : S) : sig
  open B
  (** constant false *)
